...
const (
	...
	permissionsNote = "Note: By default, all system administrators can run the `/dtrack` commands."
)
...
func (p *Plugin) executePermissions(args *model.CommandArgs, split []string) (*model.CommandResponse, *model.AppError) {
	...
	case command == "add":
		if len(split) < 2 {
			msg := "Please specify the user who needs to be whitelisted to run the /dtrack slash command. Run the command, eg: `/dtrack permissions add @user1` to whitelist a specific user."
			return p.sendEphemeralResponse(args, msg), nil
		}
		return p.handlePermissionsAdd(args, split[1])
	case command == "delete":
		if len(split) < 2 {
			msg := "Please specify the user who needs to be removed from running the /dtrack slash command. Run the command, eg: `/dtrack permissions delete @user1` to remove a specific user."
			return p.sendEphemeralResponse(args, msg), nil
		}
...
func (p *Plugin) handlePermissionsAdd(args *model.CommandArgs, username string) (*model.CommandResponse, *model.AppError) {
	...
	msg := fmt.Sprintf("User `%s` was successfully whitelisted to run `/dtrack` commands. %s", user.Username, permissionsNote)
	return p.sendEphemeralResponse(args, msg), nil
}
...
func (p *Plugin) handlePermissionsDelete(args *model.CommandArgs, username string) (*model.CommandResponse, *model.AppError) {
	...
	msg := fmt.Sprintf("User `%s` is now removed from the list of users allowed to run `/dtrack` commands. %s", user.Username, permissionsNote)
	return p.sendEphemeralResponse(args, msg), nil
}
...
func (p *Plugin) handlePermissionsList(args *model.CommandArgs) (*model.CommandResponse, *model.AppError) {
	...
	if len(perms) == 0 {
		msg := "Currently there are no users whitelisted to run `/dtrack` commands. " + permissionsNote
		return p.sendEphemeralResponse(args, msg), nil
	}

	msg := "Users whitelisted to run `/dtrack` slash commands:\n"
	for i, u := range perms {
