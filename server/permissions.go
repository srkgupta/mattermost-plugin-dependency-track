...
const (
	...
	permissionNote = "Note: By default, all system administrators can run the `/dtrack` commands."
)
...
func (p *Plugin) handlePermissionsAdd(args *model.CommandArgs, username string) (*model.CommandResponse, *model.AppError) {
	...
	msg := fmt.Sprintf("User `%s` was successfully whitelisted to run `/dtrack` commands. %s", user.Username, permissionNote)
	return p.sendEphemeralResponse(args, msg), nil
}
...
func (p *Plugin) handlePermissionsDelete(args *model.CommandArgs, username string) (*model.CommandResponse, *model.AppError) {
	...
	msg := fmt.Sprintf("User `%s` is now removed from the list of users allowed to run `/dtrack` commands. %s", user.Username, permissionNote)
	return p.sendEphemeralResponse(args, msg), nil
}
...
func (p *Plugin) handlePermissionsList(args *model.CommandArgs) (*model.CommandResponse, *model.AppError) {
	...
	if len(perms) == 0 {
		msg = "Currently there are no users whitelisted to run `/dtrack` commands. " + permissionNote
	} else {
